...
class Order extends \Core\Model{
    ...
    public function getOrderHistroy()
    {
        $conn = self::getDB();
        $query = "SELECT v.*,vi.*,u.*,b.* FROM `booking` b INNER JOIN user u ON u.id = b.userId INNER JOIN vendorinfo vi on vi.id = b.vendorinfoId INNER JOIN  vendor v ON vi.vendorId = v.id";
